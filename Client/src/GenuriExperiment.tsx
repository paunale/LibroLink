...
const Genuri: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Fetch books only if a genre is selected
    if (selectedGenre) {
      fetchBooksByGenre();
    }
  }, [selectedGenre]);
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="relative flex flex-col justify-between gap-y-2 top-28 items-center">
        <div>
          <h1 className="text-3xl pb-16 font-bold">Genuri de carti</h1>
        </div>
        {/* Genre buttons */}
            ...
            {row.map((genre) => (
              <button
                ...
                className={`w-16 h-16 rounded-full ${
                  selectedGenre === genre ? 'bg-[#E5E5CB]' : 'bg-[#E5E5CB] text-m'
                }`}
        ...
        {selectedGenre && (
          ...
          <div className="relative flex flex-row gap-y-2 items-center justify-center">
            ...
            {/* Display books */}
            <table className="relative flex flex-col justify-between gap-x-2 items-center">
              <thead>
                ...
                {books.map((book) => (
                  <tr key={book.ID} className="hover:bg-gray-50">
                    <td className="py-2 px-4 border-b">
                      {/* Use Link to navigate to another page with book details */}
